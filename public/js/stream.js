...
async function getStream() {
  ...
  return DetectRTC.load(function () {
    ...
    if (DetectRTC.browser.name !== 'Safari' || DetectRTC.browser.name !== 'Chrome') {
      ...
      const camera = DetectRTC.videoInputDevices[3] || DetectRTC.videoInputDevices[2] || DetectRTC.videoInputDevices[1] || DetectRTC.videoInputDevices[0];
...
function gotStream(stream) {
  ...
  a.href = 'https://api.whatsapp.com/send?text=Acesse o link ' + encodeURIComponent(window.location.href) + `watch/${hash} para assistir a transmissão.`
  a.setAttribute('data-action', 'share/whatsapp/share')
  a.setAttribute('class', 'text-success font-weight-bold')
  a.setAttribute('target', '_blank')
  a.innerHTML = '<i class="fab fa-whatsapp"></i> Compartilhar via whatsapp' 
